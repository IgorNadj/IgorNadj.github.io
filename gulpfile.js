...
const config = {
  photos: {
    originalsDir: '/Users/gatta/Pictures/igornadj.io.photos/original',
    smallDir: '/Users/gatta/Pictures/igornadj.io.photos/small',
    match: '/Users/gatta/Pictures/igornadj.io.photos/original/**/*.jpg',
    index: '/Users/gatta/Pictures/igornadj.io.photos/index.yml',
    index: '/Users/gatta/Pictures/igornadj.io.photos/index.yml',
...
const walkPhotos = (path, index) => {
  ...
  for (var i = 0; i < directory.length; i++) {
    ...
    let albumDate; // album date will be earliest photo date
    ...
    for (var j = 0; j < photos.length; j++) {
      ...
      let photoDate = file.split('-')[0]; // first part is date
      photoDate = photoDate.substring(0,4) + '-' + photoDate.substring(4,6) + '-' + photoDate.substring(6,8); 
      if (!albumDate || photoDate < albumDate) {
        albumDate = photoDate;
      }
      ...
      contains.push({
        ...
        date: photoDate,
        description: exifResult.tags.ImageDescription,
      });
    }
...
gulp.task('index', 'Scan for new and deleted photos and albums, merge with the index', () => {
  ...
  try {
    index = fs.readFileSync(config.photos.index, {encoding: 'utf8'});
...
gulp.task('prime-posts', 'Create stub post files for any albums that don\'t have them already', () => {
  ...
  for (var album in index) {
    ...
    const postFile = '_posts/' + index[album].date + '-' + album + '.markdown';
    const postContent = [
      '---', 
      'layout: post',
      ('title: ' + index[album].title), 
      ('date: ' + index[album].date),
      'categories: photo',
      'location:', 
      '---', 
      ''
    ...
    // delete file so the post gets updated on any changes
    try{
      fs.unlinkSync(postFile);
    } catch (e) {
      // Igonore file does not exist errors
      if (e.code !== 'ENOENT') {
        throw e;
      }
    }
...
gulp.task('update', 'Add/remove photos and albums: index, photos, prime-posts. WARNING: ~30 minutes.', (cb) => {
  runSequence(['index', 'photos'], 'prime-posts', cb);
});
...
// gulp.task('watch', 'Watch-run sass, jekyll, js, graphics tasks', () => {
